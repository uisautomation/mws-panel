...
__all__ = ["InvalidKeyException", "InvalidKeyLengthException", "TooShortKeyException", "TooLongKeyException", "InvalidTypeException", "MalformedDataException", "SSHPubKey"]
...
class SSHPubKey(object):  # pylint:disable=too-many-instance-attributes
    """
    ssh_key = SSHPubKey(key_data)
    ssh_key.parse()
    """
    ...
    def sshfp_sha1(self):
        """ Calculate sha1 sshfp record """
        return hashlib.sha1(self.decoded_key).hexdigest()

    def sshfp_sha256(self):
        """ Calculate sha1 sshfp record """
        return hashlib.sha256(self.decoded_key).hexdigest()
